...
SUMMARY_COMPACTION_PREFIX = "[GOD-code summary compaction]"
...
class SummaryCompactionStrategy(CompactionStrategy):
    def __init__(self, config: SummaryCompactionConfig) -> None:
        self._config = config

    def compact(self, messages: Messages, options: ModelOptions) -> Messages:
        del options
        copied_messages = list(messages)
        if _messages_size(copied_messages) <= self._config.max_chars:
            return copied_messages

        preserve_start = max(0, len(copied_messages) - self._config.keep_recent_messages)
        if self._config.preserve_tool_pairs:
...
def _summary_message(
    ...
    *,
    ...
    lines = [SUMMARY_COMPACTION_PREFIX, _count_line(messages)]
    snippets = _summary_snippets(messages, include_tool_results=include_tool_results)
    if snippets:
...
def _summary_snippets(messages: Messages, *, include_tool_results: bool) -> list[str]:
    snippets: list[str] = []
    for message in messages:
        kind = message.get("kind")
...
def _trim_summary(summary: JsonObject, preserved: Messages, max_chars: int) -> JsonObject:
    ...
    trimmed_content = _truncate(content, max(len(SUMMARY_COMPACTION_PREFIX), available))
...
def _is_summary_message(message: JsonMapping) -> bool:
    return message.get("kind") == "user" and isinstance(message.get("content"), str) and str(
        message.get("content")
    ).startswith(SUMMARY_COMPACTION_PREFIX)
