# Phase 197: TUI deepest bucket label visibility width percentage bucket labels

Phase197 adds human-readable `low/mid/high` labels to the deepest adaptive width percentage buckets.

## Implementation status

- Connected the existing deepest percentage-bucket label helper to the width formatter.
- Delegated label mapping to the shared legend width percentage bucket-label helper.
- Changed width-119 output to `visibility_bucket_labels_labels:adaptive>hidden+1[119/120=99%H(high)]@(`.
- Changed width-120 output to `visibility_bucket_labels_labels:adaptive>shown[120/120=100%H(high)]@(`.
