# Phase348 TUI Latest Bucket Label Width Bucket Label Visibility Width Bucket
...
在 Phase347 最新宽度百分比后追加 `L/M/H` 分段。

- 使用同层 bucket helper，并委托既有共享分段算法。
- 共享规则保持：0-39 为 `L`、40-79 为 `M`、80 及以上为 `H`。
- 80 列输出 `adaptive>hidden+40[80/120=66%H]@2`。
- 119 列输出 `adaptive>hidden+1[119/120=99%H]@2`。
- 120 列输出 `adaptive>shown[120/120=100%H]@2`。
...
后续 Phase349 已为 `L/M/H` 追加 low/mid/high 文字标签。
