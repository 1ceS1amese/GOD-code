# Phase 211: TUI deepest bucket label visibility width percentage indicators

Phase211 adds a normalized width percentage to the current-level deepest adaptive bucket-label visibility indicator.

## Implementation status

- Reused the shared clamped width-percentage helper in the current-level deepest width formatter.
- Changed width-119 output to `visibility_bucket_labels_labels_labels_labels:adaptive>hidden+1[119/120=99%]@<`.
- Changed width-120 output to `visibility_bucket_labels_labels_labels_labels:adaptive>shown[120/120=100%]@<`.
