# Phase328 TUI Latest Bucket Label Threshold
...
在 Phase327 最新 `adaptive` 标签配置提示中显式展示共享 120 列切换阈值。

- adaptive control indicator 直接复用 `LIVE_SESSION_COMMAND_NEIGHBOR_PROGRESS_BUCKET_HELP_ADAPTIVE_WIDTH`。
- 119 列基础输出 `adaptive>hidden[120]@4`；Phase329 已追加剩余距离。
- 120 列及以上输出 `adaptive>shown[120]@4`。
