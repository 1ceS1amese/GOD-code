# Phase354 TUI Latest Bucket Label Width Bucket Label Visibility Width Bucket Label Visibility Width Indicator
...
在 Phase353 adaptive 阈值距离提示中同时展示当前宽度与共享 120 列阈值。

- 新增同层 width indicator helper。
- helper 输出 `current/threshold` 并复用共享阈值常量。
- 80 列输出 `adaptive>hidden+40[80/120]@1`。
- 119 列输出 `adaptive>hidden+1[119/120]@1`。
- 120 列输出 `adaptive>shown[120/120]@1`。
