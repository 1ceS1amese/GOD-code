# Phase 223: TUI deepest bucket label visibility profiles

Phase223 upgrades the Phase222 boolean current-level deepest nested bucket-label control to explicit `shown`, `hidden`, and `adaptive` profiles.

## Implementation status

- Replaced the boolean state with a nested bucket-label visibility profile type and default `shown` state.
...
- Added `visibility_bucket_labels_labels_labels_labels_labels_labels:shown@?`, `hidden@?`, and adaptive effective indicators.
...
- `?` only cycles profiles while the command palette is open; outside the palette it remains the Help shortcut.
