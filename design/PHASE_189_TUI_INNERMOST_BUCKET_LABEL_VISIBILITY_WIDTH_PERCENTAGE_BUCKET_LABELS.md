# Phase 189: TUI innermost bucket label visibility width percentage bucket labels

Phase189 adds human-readable `low/mid/high` labels to the innermost adaptive width percentage buckets.

## Implementation status

- Added an innermost percentage-bucket label helper.
- Delegated label mapping to the shared legend width percentage bucket-label helper.
- Changed width-119 output to `visibility_bucket_labels:adaptive>hidden+1[119/120=99%H(high)]@&`.
- Changed width-120 output to `visibility_bucket_labels:adaptive>shown[120/120=100%H(high)]@&`.
