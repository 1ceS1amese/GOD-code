# Phase 199: TUI deepest bucket label visibility profiles

Phase199 upgrades the Phase198 boolean deepest bucket-label control to explicit `shown`, `hidden`, and `adaptive` profiles.

## Implementation status

- Added a deepest bucket-label visibility profile type and default `shown` state.
...
- Added `visibility_bucket_labels_labels_labels:shown@)`, `hidden@)`, and adaptive effective indicators.
