# Phase253 TUI Deepest Bucket Label Text Visibility Width Percentage Bucket Label Width Percentage Bucket Labels

## 目标

在 Phase252 的最新 `L/M/H` 分段后补充 `low/mid/high` 文字标签，使宽度区间同时具备紧凑值和可读语义。

## 已实现

- 新增 `liveSessionCommandDeepestNestedBucketLabelTextVisibilityWidthPercentageBucketLabelVisibilityWidthPercentageBucketLabel(maxWidth)`。
...
Phase254 可增加最新 `low/mid/high` 标签的独立显隐控制。
