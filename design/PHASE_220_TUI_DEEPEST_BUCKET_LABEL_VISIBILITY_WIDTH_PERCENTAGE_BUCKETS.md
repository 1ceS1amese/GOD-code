# Phase 220: TUI deepest bucket label visibility width percentage buckets

Phase220 adds the shared `L/M/H` percentage bucket to the current-level deepest nested adaptive bucket-label visibility width percentage indicator.

## Implementation status

- Connected the existing current-level percentage-bucket helper to the nested width formatter.
- Delegated bucket calculation to the shared legend width percentage bucket helper.
- Changed width-119 output to `visibility_bucket_labels_labels_labels_labels_labels:adaptive>hidden+1[119/120=99%H]@>`.
- Changed width-120 output to `visibility_bucket_labels_labels_labels_labels_labels:adaptive>shown[120/120=100%H]@>`.
