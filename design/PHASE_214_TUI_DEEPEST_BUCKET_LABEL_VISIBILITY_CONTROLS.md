# Phase 214: TUI deepest bucket label visibility controls

Phase214 adds an independent visibility control for the Phase213 current-level deepest `low/mid/high` labels while preserving the `L/M/H` bucket.
...
- Added `visibility_bucket_labels_labels_labels_labels_labels:on@>` and `off@>` indicators.
...
- Preserved width, percentage, bucket, distance, effective profile, and all outer profiles.
