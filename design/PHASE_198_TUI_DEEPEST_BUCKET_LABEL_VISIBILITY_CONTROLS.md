# Phase 198: TUI deepest bucket label visibility controls

Phase198 adds an independent visibility control for the Phase197 `low/mid/high` labels while preserving the `L/M/H` bucket.

## Implementation status

- Added default-on deepest bucket-label visibility state.
- Added a palette-local `)` toggle action and input mapping.
- Added `visibility_bucket_labels_labels_labels:on@)` and `off@)` indicators.
- Extended the deepest adaptive formatter with an independent label-visibility argument.
