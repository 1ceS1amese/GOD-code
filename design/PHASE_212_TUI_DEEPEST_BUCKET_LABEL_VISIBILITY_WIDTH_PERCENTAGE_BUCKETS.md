# Phase 212: TUI deepest bucket label visibility width percentage buckets

Phase212 adds the shared `L/M/H` progress bucket to the current-level deepest adaptive bucket-label visibility width percentage.

## Implementation status

- Connected the current-level deepest percentage-bucket helper to the width formatter.
- Delegated bucket calculation to the shared legend width percentage bucket helper.
- Changed width-119 output to `visibility_bucket_labels_labels_labels_labels:adaptive>hidden+1[119/120=99%H]@<`.
- Changed width-120 output to `visibility_bucket_labels_labels_labels_labels:adaptive>shown[120/120=100%H]@<`.
