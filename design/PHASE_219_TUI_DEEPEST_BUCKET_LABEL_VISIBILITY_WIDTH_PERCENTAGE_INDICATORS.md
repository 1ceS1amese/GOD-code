# Phase 219: TUI deepest bucket label visibility width percentage indicators

Phase219 adds a normalized width percentage to the current-level deepest nested adaptive bucket-label visibility indicator.

## Implementation status

- Reused the shared clamped width-percentage helper in the nested width formatter.
- Changed width-119 output to `visibility_bucket_labels_labels_labels_labels_labels:adaptive>hidden+1[119/120=99%]@>`.
- Changed width-120 output to `visibility_bucket_labels_labels_labels_labels_labels:adaptive>shown[120/120=100%]@>`.
