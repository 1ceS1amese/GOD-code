# Phase 226: TUI deepest bucket label visibility width indicators

Phase226 adds the current content width beside the current-level deepest nested adaptive bucket-label visibility threshold.

## Implementation status

- Added a deepest nested width helper using the shared 120-column threshold.
- Changed width-119 output to `visibility_bucket_labels_labels_labels_labels_labels_labels:adaptive>hidden+1[119/120]@?`.
- Changed width-120 output to `visibility_bucket_labels_labels_labels_labels_labels_labels:adaptive>shown[120/120]@?`.
- Preserved the Phase225 `+N` distance below the threshold.
