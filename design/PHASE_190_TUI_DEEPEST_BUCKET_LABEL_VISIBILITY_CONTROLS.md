# Phase 190: TUI deepest bucket label visibility controls

Phase190 adds an independent visibility control for the Phase189 `low/mid/high` labels while preserving the `L/M/H` bucket.

## Implementation status

- Added default-on deepest bucket-label visibility state.
- Added a palette-local `(` toggle action and input mapping.
- Added `visibility_bucket_labels_labels:on@(` and `off@(` indicators.
- Extended the innermost adaptive formatter with an independent label-visibility argument.
