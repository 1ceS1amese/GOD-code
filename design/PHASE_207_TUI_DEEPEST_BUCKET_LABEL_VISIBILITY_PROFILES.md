# Phase 207: TUI deepest bucket label visibility profiles

Phase207 upgrades the Phase206 boolean current-level deepest bucket-label control to explicit `shown`, `hidden`, and `adaptive` profiles.

## Implementation status

- Added a current-level deepest bucket-label visibility profile type and default `shown` state.
...
- Added `visibility_bucket_labels_labels_labels_labels:shown@<`, `hidden@<`, and adaptive effective indicators.
...
- `shown` always renders the current-level deepest `(low/mid/high)` label.
