# Phase313 TUI Latest Bucket Label Threshold Distance
...
在 Phase312 最新 adaptive 阈值提示中展示距离共享 120 列阈值还差多少列。

- 新增纯函数 `liveSessionCommandLatestDeepestBucketLabelTextVisibilityWidthPercentageBucketLabelVisibilityWidthPercentageBucketLabelVisibilityThresholdDistance(...)`。
- 仅当 profile 为 `adaptive` 且宽度低于 120 时返回 `120 - maxWidth`。
- 119 列输出 `adaptive>hidden+1[120]@6`。
- 120 列及以上保持 `adaptive>shown[120]@6`。
...
- Phase314 已加入当前宽度与阈值组合，例如 `[119/120]`。
